...
def AVLpolowienie(tab):
    if len(tab) == 1:
        ...
        if len(tab) > 2:  # jeśli istnieją przynajmniej 2 elementy (bo [pol] to element prawy w 2 el tablicy)
            node.right = AVLpolowienie(
                tab[pol + 1:])  # rekurencyjnie wywołujemy funkcję przypisania dla prawego dziecka
        node.treeheight = 1 + max((node.left.treeheight if node.left else 0),
                                  (node.right.treeheight if node.right else 0))
...
def deleteNode(root, val, AVL):
    if val < root.value:  # do usunięcia mniejsza niż aktualna
        ...
        if not root.left and not root.right:  # jeśli nie ma dzieci
            ...
            root.right = deleteNode(root.right, smallestinrightsub.value,
                                    AVL)  # i usuwamy podwojoną wartość w prawym poddrzewie
...
f = open("test_avl_max.txt","w")
f.write("{}\t{}\n".format("ilość danych","czas"))
#for z in range(500,3000,250):
for z in range(50000000,100000000,10000000):
    dane = []
    for x in range(z):
        dane.append(x)
